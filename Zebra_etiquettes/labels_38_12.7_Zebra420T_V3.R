...
#  Date:  27/10/2021
#  Description: la V 3.0 crée le code barre au format json et le label au format humain
#  Documentation:
#
#
#
                             ...
                             label_width = NA, label_height = NA, x_space = 0, y_space = 0.5)
{
  ...
  if (type == "linear") {
    ...
    Fsz <- ifelse(Fsz/72 > label_height * 0.3, label_height *
                    72 * 0.3, Fsz)
    label_plots <- sapply(paste0("{\"id\":\"",wsr(as.character(Labels)),"\"}"), code_128_make,
    ...
    #######ici modification pour écrire le code barre en json                                                                                                                                                    "npc"), just = c("left", "center"))
    label_plots <- sapply(paste0("{\"id\":\"",wsr(as.character(Labels)),"\"}"), qrcode_make,
  ...
  for (i in seq(1, length(label_plots))) {
    Xsplt <- Labels[i] #names(label_plots[i])
...
for (i in 1:length(ind)){
  ...
  #nom<-paste0(ind[i],echantillons[j])
